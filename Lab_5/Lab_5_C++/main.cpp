...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cin >> choice;
        cout << "Enter the coordinates for the figure in such format (x, y):" << endl;
        coordinates = input_points();
        if (choice == 1) {
            Parallelogram figure(coordinates);
            p =  figure.get_perimeter();
            sum_p_parallelograms += p;
            cout << "The perimeter of the parallelogram is " << p << endl;
        } else if (choice == 2) {
    ...
    cout << "The sum of the areas of all the squares and rectangles is " << sum_squares_rectangles << endl;
    cout << "The sum of the perimeters of all the parallelograms is " << sum_p_parallelograms << endl;
    return 0;
}
